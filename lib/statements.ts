...
export class Class extends CodeBlock {
  constructor(
    ...
    public typeParameters: TypeParameter[] = [],
    public extended: QualifiedTypeName[] = [],
    public decorators: Expression[] = []
    ) {
    super();
  }
  ...
  private buildDeclarationHead(): string {
    ...
    if (this.extended.length) {
      result += " implements " + this.extended.map(i => i.toString()).join(", ");
    }
    return result;
  }
}
...
*/
export class Spacing extends SimpleNode {
...
export class If extends CodeNode {
  ...
  buildString(): string {
    var result: string = "if (" + this.conditional.toString() + ") " + this.thenCode.toString() + this.thenCode.statementLevelTerminal();
    if (this.elseCode) result += " else " + this.elseCode.toString() + this.elseCode.statementLevelTerminal();
    return result;
  }
